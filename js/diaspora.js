...
$(function () {
  ...
  if ($('#preview').length) {
    ...
    if (cover.t.on('complete')) {
      setTimeout(function () { cover.show()}, 0) 
    }
    ...
    Diaspora.player()
    $('.icon-home, .image-icon').attr('href', '/')
    $('#top').show()
  }
  ...
  $('body').on('click', function (e) {
    ...
    switch (true) {
      ...
      case (tag.indexOf('icon-wechat') != -1):
          if ( $('#alipay').toggle().is(':hidden') == false)
              $('#alipay').toggle()
          $('#wechatpay').toggle()
        return false
        break
      case (tag.indexOf('icon-alipay') != -1):
        if ( $('#wechatpay').toggle().is(':hidden') == false)
              $('#wechatpay').toggle()
        $('#alipay').toggle()
        return false
        break
      ...
      case (rel == 'prev' || rel == 'next'):
        ...
        return false
        break
      case (tag.indexOf('toc-link') != -1):// toc
        hash = ''
        ...
        $('html,body').animate({
          scrollTop: $(decodeURI(hash)).offset().top - 50
        }, 300)
        return false
        break
      case (tag.indexOf('pviewa') != -1):            // quick view
        $('body').removeClass('mu')
        setTimeout(function () {
          Diaspora.HS($(e.target), 'push')
          $('.toc').fadeIn(1000)
